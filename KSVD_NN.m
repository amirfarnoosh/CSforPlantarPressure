...
output.totalerr=inf;
iterNum=0;
while output.totalerr(end)>param.totalerr & iterNum<param.numIteration
    iterNum=iterNum+1;
    % find the coefficients
	CoefMatrix = NN_BP(DCTm*Data, DCTm*[FixedDictionaryElement,Dictionary],param.L,CoefMatrix);
    ...
    if (iterNum>1)
        ...
        if (param.errorFlag==0 & param.displayProgress)
            disp(['Iteration   ',num2str(iterNum),'   Total error is: ',num2str(output.totalerr(iterNum-1))]);
        elseif (param.errorFlag==1 & param.displayProgress)
